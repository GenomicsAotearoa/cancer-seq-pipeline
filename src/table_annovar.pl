...
#
#
# table_annovar.pl is part of the annovar package. 
# This version of it has been modified to remove the ${build_ver}.multianno suffix from the output files
# to allow it to work with a bpipe pipeline
#
use warnings;
use strict;
use Pod::Usage;
...
our ($queryfile, $dbloc);
our ($outfile, $buildver, $remove, $checkfile, $protocol, $operation, $otherinfo, $onetranscript, $nastring, $genericdbfile, $gff3dbfile, $bedfile, $vcfdbfile, $csvout, $argument, $tempdir, $vcfinput, $dot2underline, $thread, $maxgenethread, $polishgene, $xreffile);
our $orig_command = $0;
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'remove'=>\$remove, 'checkfile!'=>\$checkfile, 
	...
	'thread=i'=>\$thread, 'maxgenethread=i'=>\$maxgenethread, 'polishgene'=>\$polishgene, 'xreffile=s'=>\$xreffile) or pod2usage ();
...
if ($vcfinput) {
	...
	backConvertVCF ("$tempfile.txt", $queryfile);
} else {
...
sub backConvertVCF {
	...
	print STDERR "-----------------------------------------------------------------\n";
	print STDERR "NOTICE: VCF output is written to $outfile.vcf\n";
...
sub printOrigOutput {
	my $final_out;
	if ($csvout) {
		$final_out="$outfile.csv";
	} else {
		$final_out="$outfile.txt";
	}
