...
TT.image<-paste0("images/",TT,".pdf")
} else {TT.image<-"/home/ben/pipeline/data/default.pdf"}
...
## Old code, referencing variable by position in a dataframe. Kept as a record to make sure I've got all the required variables
...
# hack to get around the fact that half these column names have periods in them, half have underscores, whilst trying to transition everything to _
...
S.filtered.table = S.filtered.temp %>%
	...
	rename_all(gsub, pattern = "\\.", replacement = "_") %>% # hack to get around the fact that half these column names have periods in them, half have underscores, whilst trying to transition everything to _
    	dplyr::select(one_of(tolower(report.df$varName))) %>%
	rename_at(vars(colnames(.)), ~ report.df$fieldName) %>%
...
rmarkdown::render('data/generate_report.Rmd', output_file =out1, output_dir = getwd())
}
