...
CV.tot <- matrix(data = NA, nrow = 0, 
	ncol = 31) # dummy matrix for output

# read in and filter contents of the VCF files

# set chunk size for pllel computing
len<-length(raw.snp.vcf.files)
chunks <- clusterSplit(cl, 1:len)

CV.TTot <- matrix(data = NA, nrow = 0, ncol = 31) # dummy matrix for output

time.taken <- system.time(
CV.TTot <- foreach(j=1:cores, .combine='rbind') %dopar% 
{
...
for (i in unlist(chunks[j])) { # iterate through the VCFs within the current chunk
	...
	raw.snp <- o.raw.snp[(info(o.raw.snp)$SS == 2)] # remove all but putative somatic mutations
...
CV.tot # return local result for this chunk
} # end doPar loop
) # end system.time
