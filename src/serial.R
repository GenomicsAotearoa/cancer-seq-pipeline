...
## Amended/adapted to bpipe, Ben Curran. 
## Comments by Ben : ##
## Comments by Cris : #
...
## So. The problem with this script and the pipeline is that it doesn't fall over gracefully. 
## The edge case is that if the filters are sufficiently strict that there is only a single 
## record found, then the writing of data to file will fall over. It shouldn't. 
## It entirely possible that this won't crop up very often, but ...
## It also doesn't fall over gracefully if there are no records left after filtering ...
...
## This needs to be turned back on. Or better yet, taken out of here and given it's own step
## in the pipeline. Or at least a separate execution. It's output has no bearing on anything else
## that's made in this file. 
...
# this is hard coding in the metadata location. Which is bad. 
metadata <- read.table("reports/somatic/scripts/metadata.txt", skip = 0, sep = "\t", as.is = TRUE)
colnames(metadata) <- metadata[1, ]
metadata <- metadata[-1, ]

#raw.snp.vcf.files <- list.files(path = "." , pattern = ".vcf$") # read in file list
#print(raw.snp.vcf.files)

CV.tot <- matrix(data = NA, nrow = 0, ncol = 31) # dummy matrix for output

# read in and filter contents of the VCF files
## This should be changed to used the pre and post filters in the Variant Annotation package. I've done this elsewhere, 
## I just haven't had time to incorporate it yet. 
o.raw.snp = readVcf(rawVCFfile,"hg19")
raw.snp <- o.raw.snp[(info(o.raw.snp)$SS == 2)] # remove all but putative somatic mutations and LoH
raw.snp <- raw.snp[(geno(raw.snp)$AD[, 2] + geno(raw.snp)$RD[, 2]) >= Tdepth]
raw.snp <- raw.snp[(geno(raw.snp)$AD[, 	1] + geno(raw.snp)$RD[, 1]) >= 	Ndepth]
raw.snp <- raw.snp[(geno(raw.snp)$AD[, 	2] >= T.var.depth)]
raw.snp <- raw.snp[(100 * geno(raw.snp)$AD[, 2]/(geno(raw.snp)$AD[, 2] + geno(raw.snp)$RD[, 2])) >= (T.var.per)]
#raw.snp <- raw.snp[(geno(raw.snp)$AD[, 1] == 0)] # no ALT reads in germline
...
# write filtered VCF
filename=paste0("trimmed_",rawVCFfile)
writeVcf(raw.snp, filename)
...
## Various cleanings of the various data. 
supl <- info(raw.snp)$SUPPLEMENTARY
## not sure what supl used to come in as. It's currently coming in as NULL, so I'm swapping 
## the length ==1 test for !is.null.
#if (length(supl) == 1) { 
if (!is.null(supl)){
	supl <- "YES"
} else {
...
if (nrow(raw.snp) > 0) {
    reps = nrow(raw.snp)
    CV.out =  cbind(rep(rawVCFfile, nrow(raw.snp)), rep(ii, nrow(raw.snp)),
        lapply(info(raw.snp)$Gene.refGene, `[`, 1),
        ij, SPV, positions, clinvar, cosmic, CosCensusSomatic, 
        CosCensusGermline, ACMG, popfreqmax, ExAc_ALL, gnomAD_exome_ALL, 
        gnomAD_genome_ALL, muttype, gene.position, num.germline.REF, 
        num.germline.ALT, num.somatic.REF, num.somatic.ALT, fract.somatic.ALT, 
        snp, repeat_mask, ENCODE.Dac.unmappable, segdup, supl, sift, MutationTaster, 
         MetaLR, FATHMM)

    names = c("tumour","Indel or SNP", "Main gene", "Downstream gene", "Somatic_P_value", "Position", "Variant position in ClinVar", "Variant position in Cosmic v82", 
			"In Cosmic Census Somatic Nov17", "In Cosmic Census Germline Nov17", "Variant gene in ACMG 57 gene list?", "Maximum Population Frequency", "ExAc All", 
            "gnomAD exome All", "gnomAD_genome All", "Mutation type", "ANNOVAR gene position", "num Germline REF reads", "num Germline ALT reads", "num Somatic REF reads", 
			"num Somatic ALT reads", "Fraction of somatic reads that are ALT", "Variant position in dbSNP147 ?", "Variant position in repeat masked region?",
            "Variant position in  ENCODE Dac unmappable region?", "Variant position in regions of segmental duplication?", "Variant supported by supplementary reads?", 
            "Variant effect - SIFT", "Variant effect - Mutation_Taster", "Variant effect - MetaLR", "Variant effect - FATTHM")
    colnames(CV.out) = names
	CV.tot <- rbind(CV.tot, CV.out)
}

## setting this is a temp fix, until I get the BAF plot thing to work. 
BAF.plot="n"

if (BAF.plot =="y") {	
	if (length(grep("snp", raw.snp.vcf.files[i])) == 1) { # a SNP VCF (not an Indel VCF)
		# then plot B allele frequency
		#==============
        raw.snp <- o.raw.snp[(geno(o.raw.snp)$AD[, 2] + geno(o.raw.snp)$RD[, 2]) >= 20]
		raw.snp <- raw.snp[(geno(raw.snp)$AD[, 1] + geno(raw.snp)$RD[, 1]) >= 20]
		raw.snp <- raw.snp[unlist(info(raw.snp)$Func.refGene == "exonic") | unlist(info(raw.snp)$Func.refGene == "splicing")]
		Graw.snp <- raw.snp[(pbinom(pmin(as.numeric(geno(raw.snp)$RD[, 	1]), as.numeric(geno(raw.snp)$AD[, 1])), as.numeric(geno(raw.snp)$AD[, 1] + geno(raw.snp)$RD[, 1]), 0.5)) >= 0.05 & (100 * geno(raw.snp)$AD[, 1]/(geno(raw.snp)$AD[, 1] + geno(raw.snp)$RD[, 1])) <= 70 & (100 * geno(raw.snp)$AD[, 1]/(geno(raw.snp)$AD[, 1] + geno(raw.snp)$RD[, 1])) >=30]
		#==============
		a.snp <- strsplit(unlist(rowRanges(Graw.snp)@ranges@NAMES), ":")
		chr.snp <- unlist(lapply(a.snp, `[`, 1))
		pos.snp <- unlist(lapply(strsplit(unlist(lapply(a.snp, `[`, 2)), "_"), `[`, 1))
		gn <- unlist(lapply(info(Graw.snp)$Gene.refGene, `[`, 1))
		Tsum <- sum(geno(Graw.snp)$AD[, 2] + geno(Graw.snp)$RD[, 2])
		Nsum <- sum(geno(Graw.snp)$AD[, 1] + geno(Graw.snp)$RD[, 1])
		norm.ratt <- Tsum/Nsum
		Treads <- (geno(Graw.snp)$AD[, 2] + geno(Graw.snp)$RD[, 2])
		NreadsADJ <- (geno(Graw.snp)$AD[, 1] + geno(Graw.snp)$RD[, 	1]) * norm.ratt
		read.ratio <- Treads/NreadsADJ
		read.ratio <- read.ratio * 2 # x2 to make the ratio equivalent to copy number
		ALTratio <- geno(Graw.snp)$AD[, 2]/Treads
		forDC <- data.frame(chromosome = chr.snp, start = pos.snp, stop = pos.snp, 	x = ALTratio, y = read.ratio, name = gn)
		forDC$start <- as.character(forDC$start)
		forDC$stop <- as.character(forDC$stop)
		forDC$chromosome <- as.character(forDC$chromosome)
		forDC$chromosome <- gsub("chr", "", forDC$chromosome, fixed = TRUE)
		cf <- as.numeric(forDC$x)
		cf[cf > 0.5] <- 1 - cf[cf > 0.5]
		CNA.object <- CNA(as.numeric(cf), as.numeric(forDC$chromosome), as.numeric(forDC$start), data.type = "logratio")
...
P.CGI<-sub(":", " ", as.character(CV.totd$Position))
P.CGI<-sub("_", " ", as.character(P.CGI)); P.CGI<-sub("/", " ", as.character(P.CGI))
S.CGI<-paste(CV.totd$tumour, CV.totd$Position, sep="_")
dfv<-t(data.frame(strsplit(P.CGI, " ")))
chr.mut<-dfv[,1]; start.mut<-dfv[,2]
end.mut<-as.numeric(start.mut) - 1 + nchar(dfv[,4])
...
if (class(CV.totf) == "list"){
    nameList = names(CV.totf)
    CV.totf = t(matrix(CV.totf, ncol=1))
    colnames(CV.totf) = nameList
}else{
    CV.totf = as.matrix(CV.totf)
}
# then write
rownames(CV.totf) <- NULL
file <- paste(date(), "somatic_indel_OR_snv.txt", sep = "_")
write.table(t(colnames(CV.totf)), file = file,	sep = "\t", row.names = "index", col.names = FALSE, append = TRUE)
write.table(CV.totf, file = file, sep = "\t", row.names = TRUE, col.names = FALSE,	append = TRUE)

# write file with no variants supported by supp reads
## We're not doing this. Or at least, in the intial stages of the pipeline. Or are we?
CV.totf.noSupp<-CV.totf[CV.totf[,27]==".",]
rownames(CV.totf.noSupp) <- NULL
file <- paste(date(), "NO.SUP.somatic_indel_OR_snv.txt", sep = "_")
write.table(t(colnames(CV.totf.noSupp)), file = file, 	sep = "\t", row.names = "index", col.names = FALSE, append = TRUE)
write.table(CV.totf.noSupp, file = file, sep = "\t", row.names = TRUE, col.names = FALSE, 	append = TRUE)
...
file <- paste(date(), "ForCGI_NO.SUP.somatic_indel_OR_snv.txt", sep = "_")
write.table(t(colnames(CV.totf.noSupp.CGI)), file = file, sep = "\t", row.names = F, col.names = FALSE, append = TRUE)
write.table(CV.totf.noSupp.CGI, file = file, sep = "\t", row.names = F, col.names = FALSE, 	append = TRUE)
